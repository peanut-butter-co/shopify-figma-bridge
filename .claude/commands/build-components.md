...
**Instance lookup table:** Build this from the atoms confirmed in `/propose-components`. For every atom in `components.atoms`, map common sub-element patterns to their component name. Example entries:
...
| Navigation arrow | Arrow Button (Left/Right) |

**Before building ANY composite:** List sub-elements needed. For each, check if the component exists in `components.atoms`. If yes → instance. If no → build the atom first.
